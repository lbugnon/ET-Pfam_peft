...
class PFamDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.is_training:
            if item.end-self.window_len//2+1<=item.start+self.window_len//2:
                center = item.start+self.window_len//2 
            else:
                center = np.random.randint(item.start+self.window_len//2, item.end-self.window_len//2+1)
        else:
            center = (item.start + item.end)//2

        start = max(0, center - self.window_len//2)
        end = min(L, center + self.window_len//2)
